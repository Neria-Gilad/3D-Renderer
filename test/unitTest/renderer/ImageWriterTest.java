...
/**
 * Created by Neria Tzidkani on 16/05/2017.
 */
    ...
    public void neriaTest_switzerland() {
        ...
        for (int i = 0; i < height; i++)
            for (int j = 0; j < width; j++)
                if ((i > height * 2 / 5 && i < height * 3 / 5) || (j > width * 2 / 5 && j < width * 3 / 5))
                    imageWriter.writePixel(j, i, Color.white);  // White
                else
                    imageWriter.writePixel(j, i, Color.red);  // Red

        for (int i = 0; i < height; i++)
            for (int j = 0; j < width; j++)
                if ((i < height / 5) || (i > height * 4 / 5) || (j < width / 5) || (j > width * 4 / 5))
